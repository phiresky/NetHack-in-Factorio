...
- ~~Blocking import protocol~~ FIXED: yn_function/getlin are now non-blocking,
  nhgetch/select_menu are blocking. Input queue drains getlin chars and menu IDs.
- ~~Missing info.json~~ FIXED: mod manifest created.
...
- **Performance**: May need to increase `MAX_INSTRUCTIONS_PER_RUN` (currently 50K)
  for NetHack's level generation which can be millions of instructions.
