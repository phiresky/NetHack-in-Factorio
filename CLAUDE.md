...
## Building
```bash
./build.sh              # full build (clone + host tools + tilemap + WASM + sprites)
./build.sh --wasm-only  # recompile WASM + regenerate Lua modules
...
build.sh                    — Top-level build script (full pipeline + convenience commands)
...
- **Tile system**: `glyph2tile[]` (from `tile.c`) maps NetHack glyph indices to
  ...
  Old ASCII letter entities (nh-mon-a etc.) replaced by nh-mon-{0..788} etc.
...
- **Compile**: `./build.sh` (clones NetHack, builds host tools, cross-compiles to WASM,
...
- **Tile sprites**: Real NetHack pixel art tiles replace ASCII placeholders. Old
  entity PNGs in `graphics/entities/` (nh-mon-a.png etc.) are dead files — only
  `nh-player-marker.png` is still used (as shared icon for all entities).
  `convert_tiles.py` generates the real tile sprites from NetHack source art.
