...
Build-time:  NetHack 3.6.7 C -> host tools (makedefs/lev_comp/dgn_comp, -m32)
             ...
             -> Emscripten -> nethack.wasm (2MB) -> embed as Lua data (8MB)
...
cd ../build && EM_PYTHON=/usr/bin/python3 make
...
- **NOMAIL object index mismatch**: `-DNOMAIL` in emcc excludes the mail scroll from
  ...
  use `-DNOMAIL` in emcc — keep defines consistent between native and cross builds.
...
- **K&R C compatibility**: NetHack 3.6 uses K&R-style function declarations.
  `system.h` has parameterless declarations like `void srand48()` that conflict with
  modern glibc full prototypes. Fix: compile with `-std=gnu89`.
...
- **NetHack 3.6.7 WASM runs**: Starts in ~2M instructions (down from ~59M with 3.7),
  reaches first input prompt. WASM binary is 2MB (no embedded Lua 5.4 interpreter).
...
- **Performance**: Startup takes ~2M instructions. May still need to tune
  `MAX_INSTRUCTIONS_PER_RUN` and `MAX_INSTRUCTIONS_PER_TICK` for smooth gameplay.
