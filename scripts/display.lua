...
-- Uses tile_idx from glyph2tile[] for unique sprite selection, plus ch/color/special for behavior.
local Display = {}
...
function Display.print_glyph(x, y, tile_idx, ch, color, special)
  ...
  if WALL_CHARS[ch] then
    surface.set_tiles({{name = "nh-floor", position = {x = x, y = y}}})
    place_entity(surface, level_name, x, y, ent_name, nil)
    return
  end
  ...
  -- Dungeon features (floor, corridor, void)
  ...
  -- Ice: . with CLR_CYAN
  if ch == string.byte(".") and color == 6 then
  ...
  -- Items and other visible features (anything non-space above ASCII 32)
